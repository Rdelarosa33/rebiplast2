...
export default async function SupervisorPage() {
  ...
  const [{ data: pendientesRecibir }, { data: pendientesAsignar }, { data: enCalidad }, { data: listosEntrega }] = await Promise.all([
    supabase.from('piezas').select('*, siniestro:siniestros(numero_siniestro,placa,taller_origen)')
      .eq('estado', 'EN_TRASLADO').order('created_at'),
    supabase.from('piezas').select('*, siniestro:siniestros(numero_siniestro,placa,taller_origen)')
      .eq('estado', 'RECIBIDO').order('created_at'),
    supabase.from('piezas').select('*, siniestro:siniestros(numero_siniestro,placa,taller_origen)')
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-[#475569] mt-0.5">Gestiona recepciones, asignaciones y control de calidad</p>
      </div>

      {/* KPIs rápidos */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: 'Por recibir', value: pendientesRecibir?.length || 0, color: 'text-blue-400' },
          { label: 'Por asignar', value: pendientesAsignar?.length || 0, color: 'text-violet-400' },
          { label: 'En calidad', value: enCalidad?.length || 0, color: 'text-purple-400' },
          { label: 'Listo entrega', value: listosEntrega?.length || 0, color: 'text-green-400' },
        ].map(k => (
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        <Section title="Pendientes de recibir" piezas={pendientesRecibir} icon={Package}
          color="text-blue-400" emptyMsg="No hay piezas en traslado" />
        <Section title="Por asignar a trabajador" piezas={pendientesAsignar} icon={Clock}
          color="text-violet-400" emptyMsg="No hay piezas recibidas sin asignar" />
        <Section title="Control de calidad" piezas={enCalidad} icon={ShieldCheck}
          color="text-purple-400" emptyMsg="No hay piezas en control de calidad" />
        <Section title="Listo para entrega" piezas={listosEntrega} icon={CheckCircle}
          color="text-green-400" emptyMsg="No hay piezas listas para entrega" />
      </div>
    </div>
  )
}
