...
import { ArrowLeft, CheckCircle, AlertCircle, User, QrCode } from 'lucide-react'
...
export default function ScanPiezaPage({ params }: { params: { id: string } }) {
  ...
  const ejecutarAccion = async (accion: Accion, motivoText?: string) => {
    ...
    if (result.error) {
      ...
      setSuccess(`✓ Pieza actualizada a: ${ESTADO_LABELS[accion.estado_nuevo]}`)
    ...
    <div className="max-w-lg space-y-4">
      ...
      <div className="card p-4">
        ...
        <div className="grid grid-cols-2 gap-3 mt-3 text-sm">
          <div>
            <p className="text-xs text-[#475569]">Lado</p>
            <p className="text-white">{pieza.lado || '—'}</p>
          </div>
          <div>
            <p className="text-xs text-[#475569]">Color</p>
            <p className="text-white">{pieza.color || '—'}</p>
          </div>
        </div>
      ...
      {(pieza.trabajador_reparacion_nombre || pieza.trabajador_pintura_nombre) && (
        <div className="card p-4 space-y-2">
          ...
          {pieza.trabajador_reparacion_nombre && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-[#94A3B8]">Reparación:</span>
              <span className="text-xs text-white">{pieza.trabajador_reparacion_nombre}</span>
            </div>
          )}
          {pieza.trabajador_preparacion_nombre && (
            <div className="flex items-center gap-2">
              <User size={14} className="text-orange-400" />
              <span className="text-xs text-[#94A3B8]">Preparación:</span>
              <span className="text-xs text-white">{pieza.trabajador_preparacion_nombre}</span>
            </div>
          )}
          {pieza.trabajador_pintura_nombre && (
            <div className="flex items-center gap-2">
              <User size={14} className="text-pink-400" />
              <span className="text-xs text-[#94A3B8]">Pintura:</span>
              <span className="text-xs text-white">{pieza.trabajador_pintura_nombre}</span>
            </div>
          )}
        </div>
      )}
      ...
      {profile && (
        <div className="card p-4 space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold text-[#475569] uppercase tracking-wider">Acciones disponibles</p>
            <span className="text-xs px-2 py-0.5 rounded-full bg-[#131920] text-[#94A3B8]">
              {ROLE_LABELS[profile.role]}
            </span>
          </div>
          {acciones.length === 0 ? (
            <p className="text-sm text-[#475569] text-center py-2">No hay acciones para tu rol en este estado</p>
          ) : (
            <div className="space-y-2">
              {acciones.slice(0, 6).map((accion, i) => (
        ...
        <div className="fixed inset-0 bg-black/70 flex items-end sm:items-center justify-center z-50 p-4">
          <div className="card p-5 w-full max-w-sm space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => ejecutarAccion(modal, motivo)} disabled={!motivo || accionLoading}
                className="btn-danger flex-1 justify-center">Confirmar</button>
            </div>
          </div>
        </div>
      )}
