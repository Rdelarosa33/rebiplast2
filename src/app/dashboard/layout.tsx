...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    ...(profile?.role === 'admin' || profile?.role === 'supervisor' || profile?.role === 'recojo'
    ...
    ...(profile?.role === 'supervisor' || profile?.role === 'admin'
      ...
      : []),
    ...(['reparacion','preparacion','pintura'].includes(profile?.role || '')
      ? [{ href: '/trabajador', icon: Hammer, label: 'Mis Piezas' }]
      : []),
    ...(profile?.role === 'recojo' || profile?.role === 'admin'
