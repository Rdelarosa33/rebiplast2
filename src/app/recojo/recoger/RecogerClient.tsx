...
import { ArrowLeft, Search, Truck, Check, AlertCircle, Building2 } from 'lucide-react'
import { recogerPiezasDelTaller } from '../actions'
...
export default function RecogerClient({ piezas }: { piezas: Pieza[] }) {
  ...
  const confirmar = () => {
    ...
    startTransition(async () => {
      try {
        ...
        await recogerPiezasDelTaller(ids)
        router.push('/recojo')
        router.refresh()
      } catch (e: any) {
        setError(e.message)
  ...
  return (
    <div className="container max-w-3xl mx-auto p-4 space-y-4 pb-32">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h1 className="text-xl font-bold text-white">Recoger del taller</h1>
          <p className="text-xs text-[#94A3B8]">{piezas.length} pieza{piezas.length !== 1 ? 's' : ''} pendiente{piezas.length !== 1 ? 's' : ''}</p>
        </div>
      </div>
      ...
      {agrupadasPorTaller.length === 0 ? (
        <div className="bg-[#0D1117] border border-[#1E2D42] rounded-xl p-8 text-center">
          <Truck className="mx-auto text-[#475569] mb-3" size={32} />
          <p className="text-sm text-[#94A3B8]">
            {busqueda ? 'Sin resultados' : 'No hay piezas pendientes de recoger'}
          </p>
        </div>
      ) : (
                  ...
                  >
                    ...
                    <div className="flex-1 text-left">
                      ...
                      <p className="text-[10px] text-[#475569]">
                        OC: {sin.numero_orden || '-'} · {piezasSin.length} pieza{piezasSin.length !== 1 ? 's' : ''}
                      </p>
                    </div>
                  </button>
                    ...
                    {piezasSin.map(p => (
                      ...
                      >
                        <div className={`w-3.5 h-3.5 rounded border flex-shrink-0 flex items-center justify-center ${
                          seleccionadas.has(p.id) ? 'bg-[#00D4FF] border-[#00D4FF]' : 'border-[#475569]'
                        }`}>
                        ...
                        <span className="text-[10px] text-[#00D4FF] font-mono">{p.tipo_trabajo}</span>
                      </button>
                    ))}
                  </div>
                </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-[#080B12] border-t border-[#1E2D42] p-3">
          <div className="container max-w-3xl mx-auto">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                <Truck size={14} />
                {pending ? 'Procesando...' : `Traer a Base (${seleccionadas.size})`}
              </button>
            </div>
          </div>
        </div>
